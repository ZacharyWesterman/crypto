...
def reprocessArgs(args):
    if not (args.save or args.text or args.view): args.view = True
    if not args.analyzers: args.analyzers = ['wordSearch']
    elif 'all' in args.analyzers: args.analyzers = list(analyzers.keys())
    return args

def main():
    timer, parser = Timer(), argparse.ArgumentParser(description="Plot score distributions from analyzers.")
    ...
    for d in (getSubDicts(scoreDict) if args.recursive else [scoreDict]):
